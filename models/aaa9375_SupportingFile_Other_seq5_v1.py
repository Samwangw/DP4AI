...
def runClassifier(n, d, krange, X_train, X_holdout, X_test):
    ...
    vals = []
    trainanswers = np.dot(X_train[:, range(0, d)].T, X_train[:, d]) / n
    holdoutanswers = np.dot(X_holdout[:, range(0, d)].T, X_holdout[:, d]) / n
...
def plot1(x, mean, std, plotname, plottitle, legend_pos=2):
    ...
    plt.fill_between(x, mean[:, 0] - std[:, 0], mean[:, 0] + std[:, 0], alpha=0.5, edgecolor='#B2B2F5',
                     facecolor='#B2B2F5', linestyle='dashdot')
    plt.plot(x, mean[:, 1], 'x-', label='holdout', color='#006600')
    plt.fill_between(x, mean[:, 1] - std[:, 1], mean[:, 1] + std[:, 1], alpha=0.5, edgecolor='#33CC33',
                     facecolor='#CCFFCC', linestyle='dashdot')
    plt.plot(x, mean[:, 2], 'r|-', label='fresh')
    plt.fill_between(x, mean[:, 2] - std[:, 2], mean[:, 2] + std[:, 2], alpha=0.5, edgecolor='#CC4F1B',
                     facecolor='#FF9848', linestyle='dashdot')
...
def plot2(x, mean, std, mean2, std2, plotname, plottitle, legend_pos=2):
    fig = plt.figure(1, figsize=(6.5, 4))
    plt.title(plottitle, fontsize='14')
    plt.ylabel('error', fontsize='14')
    plt.xlabel('number of variables', fontsize='14')
    plt.axis([x[0], x[-1], 0.0, 0.75])

    plt.plot(x, mean[:, 1], 'x-', label='std-holdout', color='#006600')

    plt.plot(x, mean2[:, 1], 'x-', label='thresholdout', color='#FF9848', linestyle=':')
...
def runandplotsummary(n, d, krange, reps, datafn, plotname):
    ...
    plot2(krange, mean, std, mean2, std2, plotname + "-compare", "Compare")
